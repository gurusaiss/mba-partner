        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "20px" }}>
          {steps.map((s) => (
            <div key={s.n} className="card" style={{ padding: "28px", display: "flex", flexDirection: "column", gap: "16px" }}>
              <div className="serif" style={{ fontWeight: 900, fontSize: "2.8rem", lineHeight: 1, color: "rgba(201,168,76,0.18)" }}>{s.n}</div>
              <h3 className="serif" style={{ fontWeight: 700, fontSize: "1.05rem", color: "var(--text)" }}>{s.title}</h3>
              <p style={{ fontSize: "0.95rem", lineHeight: 1.7, color: "var(--muted)" }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
