    ...
    <section id="resources" style={{ padding: "96px 0", background: "var(--card)" }}>
      <div style={{ maxWidth: "1152px", margin: "0 auto", padding: "0 40px" }}>
        ...
        <div style={{ marginBottom: "56px" }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: "1rem" }}>Every resource is curated by alumni who went through the exact same process. Nothing generic.</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px" }}>
          {resources.map(r => (
            <div key={r.title} className="card" style={{ padding: "28px", display: "flex", flexDirection: "column" }}>
              ...
              <h3 className="serif" style={{ fontWeight: 700, fontSize: "1rem", color: "var(--text)", marginBottom: "10px" }}>{r.title}</h3>
              <p style={{ fontSize: "0.875rem", lineHeight: 1.65, color: "var(--muted)", flex: 1, marginBottom: "20px" }}>{r.desc}</p>
              <div style={{ fontSize: "0.78rem", fontWeight: 600, paddingTop: "16px", borderTop: "1px solid var(--border)", color: "var(--gold)" }}>
                {r.count}
              </div>
            </div>
          ))}
