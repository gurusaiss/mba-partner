...
const courses = [
  ...
  { id: 2, category: "case", tag: "tag-green", tagLabel: "Case Competition", title: "Case Competition Bootcamp", desc: "Coached by AIR 1. 4 live sessions (2 hrs each), 30+ winning PPTs, Canva Premium for 1 year, and a national competition calendar.", price: "₹3,499", original: "₹6,000", points: ["4 live sessions × 2 hrs", "30+ winning case PPTs", "Canva Premium — 1 year"], link: "https://www.mbapartner.in/product/case-competition-bootcamp/", featured: false },
    ...
    <section id="courses" style={{ padding: "96px 0" }}>
      <div style={{ maxWidth: "1152px", margin: "0 auto", padding: "0 40px" }}>
        ...
        <div style={{ marginBottom: "40px" }}>
          ...
          <h2 className="serif" style={{ fontSize: "clamp(1.8rem, 3vw, 2.6rem)", fontWeight: 900, lineHeight: 1.15, letterSpacing: "-0.02em", color: "var(--text)", marginBottom: "10px" }}>
            Courses & Packages
          </h2>
          <p style={{ color: "var(--muted)", fontSize: "1rem" }}>Transparent pricing. Real deliverables. No upsells.</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px" }}>
          {filtered.map(c => (
            <div key={c.id} className={`card ${c.featured ? "card-featured" : ""}`} style={{ padding: "28px", display: "flex", flexDirection: "column" }}>
              ...
              <h3 className="serif" style={{ fontWeight: 700, fontSize: "1rem", color: "var(--text)", marginBottom: "10px" }}>{c.title}</h3>
              <p style={{ fontSize: "0.875rem", lineHeight: 1.65, color: "var(--muted)", marginBottom: "20px", flex: 1 }}>{c.desc}</p>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginTop: "auto" }}>
                <div>
                  <div className="serif" style={{ fontWeight: 900, fontSize: "1.5rem", color: "var(--gold)" }}>{c.price}</div>
                  <div style={{ fontSize: "0.75rem", textDecoration: "line-through", color: "var(--dim)" }}>{c.original}</div>
                </div>
